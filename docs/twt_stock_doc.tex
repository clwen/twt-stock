...
Until the time this report was written, we have collected tweets from Twitter API from xxx to xxx. The number of tweets we collected for each company and keyword are summarized in the following table.

The number of objective tweets collected from technology news and subjective 

\subsection{Subjectivity Classification}
Accuracies of different classifiers are summarized in the following table. Since xxx procues the highest result, we use it to produce classification model by using the following command in WEKA:

Then, the model is used to classify tweets in testing sets.

The result of subjective classfication can be summarized as the following table:
...
We collected the corresponding stock prices from Yahoo! Fianance. The following plots give the reader an overview of the relationship between ratio of sentiment analysis and its stock prices.

If we run Pearson Coeficient tests on this two vectors, the results can be summarized as the following.

As we can see, there are high correlated companies as well as the neutral or even negatively correlated companies.
